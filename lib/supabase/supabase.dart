...
class SupabaseProvider extends ChangeNotifier {
  ...
  ComplexFoodTable() async {
    final uid = supabase.auth.currentUser!.id;

    final response = await supabase.from('users').select('''
    food( 
      complex_food!complex_food_food_id_food_fkey(
          id_complex_food,
          food_id_food,
          ingredient_food_id_food,
          grams,
          created_at,
          ended_at
      )
    ...
    final List<dynamic> userData = response as List<dynamic>;
    List<ComplexFood> complexFoodDataList = [];
    print("********************************************************************************************");

    userData.forEach((user) {
      List<dynamic> foods = user['food'] ?? [];

      foods.forEach((food) {
        List<dynamic> complexFoods = food['complex_food'] ?? [];
        complexFoods.forEach((data) {
          if (data != null && data is Map<String, dynamic>) {
            ComplexFood complexFoodDatatItem = ComplexFood.fromJson(data);
            complexFoodDataList.add(complexFoodDatatItem);
          }
        });
      });
    });
